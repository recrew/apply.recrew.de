<script lang="ts">
    ...
    const bindLicense1 = (ev: CustomEvent) => {
        employee.images[licenseIndex].name = fileNameGenerator(
            ev.detail.file,
            employee,
            "license",
            "Vorderseite",
    ...
    const bindLicense2 = (ev: CustomEvent) => {
        const preExisting =
            employee.images.filter((n) => n.imageTag === "license").length > 1;
        const name = fileNameGenerator(
            ev.detail.file,
            employee,
            "license",
            "Rückseite",
        ...
        if (!preExisting) {
            employee.images = [
                ...employee.images,
                { documentNumber: null, imageTag: "license", file: null, name },
            ];
        }
        const index = employee.images.findIndex(
            (n) => n.imageTag === "license" && n.documentNumber === null,
        );
        employee.images[index].name = name;
        employee.images[index].file = ev.detail.file;
    };
    const bindStudentVerification = (ev: CustomEvent) => {
        const preExisting = employee.images.findIndex(
            (n) => n.imageTag === "student-verification",
        );
        const name = fileNameGenerator(
            ev.detail.file,
            employee,
    ...
    const bindHealthCert = (ev: CustomEvent) => {
        ...
        employee.images[idx].name = fileNameGenerator(
            ev.detail.file,
            employee,
            tag,
        );
    ...
    $: {
        healthCertificateIndex = employee.images.findIndex(
            (i) => i.imageTag === "health-certificate",
        );
    ...
    $: {
        if (
            healthCertificateIndex >= 0 &&
            employee.images[healthCertificateIndex]?.issueDate
        ) {
            ...
            if (
                ...
                issueDate.isValid() &&
                issueDate.isBefore(threeMonthsAgo)
            ) {
    ...
    $: {
        if (employee.cv.motorVehicleLicense) {
            licenseIndex = employee.images.findIndex(
                (n) => n.imageTag === "license",
            );
            if (licenseIndex < 0) {
                employee.images = [
                    ...employee.images,
                    { documentNumber: "", imageTag: "license", file: null },
                ];
...
>
    <div class="grid md:grid-cols-2 gap-y-3 gap-x-4 mt-2">
                ...
                options={[
                    {
                        ...
                        value: "student-verification",
                    },
                ]}
                noRead={true}
                on:ocr={bindStudentVerification}
            />
        {/if}
        <div>
        ...
        {#if employee.cv.motorVehicleLicense}
            <Tesseract
                value={employee.images.find(
                    (n) => n.imageTag === "license" && n.documentNumber,
                )}
                options={[
                    { name: "Führerschein Vorderseite", value: "license" },
                ]}
                on:ocr={bindLicense1}
            />
            <div>
                ...
                <Input
                    id="license"
                    bind:value={employee.images[licenseIndex].documentNumber}
                    required
                />
            ...
            <Tesseract
                ...
                options={[{ name: "Führerschein Rückseite", value: "license" }]}
                noRead={true}
                on:ocr={bindLicense2}
            />
        {/if}
    ...
    <div class="mt-6">
        <Tesseract
            value={employee.images.find(
                (n) => n.imageTag === "health-certificate" && n.issueDate,
            )}
            options={[
                { name: "Gesundheitszeugnis", value: "health-certificate" },
            ]}
        ...
        {#if healthCertificateIndex >= 0 && employee.images[healthCertificateIndex]?.file}
            <div class="mt-3">
                <Label for="certificateDate" class="mb-2"
        ...
        {#if healthCertificateError}
            <Alert color="red" class="mt-3">
                ...
                <p class="text-sm">
                    Das Ausstellungsdatum des Gesundheitszeugnisses darf maximal
                    3 Monate zurück liegen.<br />
        ...
        {:else if healthCertificateIndex >= 0 && !employee.images[healthCertificateIndex]?.file}
            <Alert border color="yellow" class="mt-3">
                <InfoCircleSolid slot="icon" class="w-5 h-5" />
                <p class="text-sm">
                    Du kannst das Gesundheitszeugnis nachreichen, musst es
                    jedoch <strong>spätestens</strong> zum Get-to-Know-Treffen
                    vorlegen.<br />
                    Das Gesundheitszeugnis lässt sich schnell und unkompliziert online
                    beantragen.<br />
                    <a
                        href="https://www.google.com/search?q=gesundheitszeugnis+online"
