...
export const readIdCard = (parsedText: string) => {
    const lines = parsedText.split("\n");
    lastLine = ''
    lines.forEach(line => {
        if (lastLine.includes('BUNDESREPUBLIK')) {
            passportNumber = line.match(/\s(^(?=.*[0-9])[A-Z0-9]{9})/)?.[1] ?? '';
        } else if (lastLine.includes('Geburtsname/Name')) {
            lastName = capitalize(line.trim().match(/[A-Z-\s]{3,}/)?.[0] ?? '');
            maidenName = lastName;
        } else if (lastLine.includes('Vornamen')) {
            firstName = capitalize(line.trim());
        } else if (lastLine.includes('Date de naissance')) {
