<script lang="ts">
    ...
    let candidate = {
        ...
        application_field:
            $page.url.searchParams.get("application") || "Service",
        letter_motivation: "",
        photo: null,
        reCapRes: "",
            ...
            [...event.dataTransfer.items].forEach(async (item, i) => {
                if (item.kind === "file") {
                    if (item.getAsFile().type === "application/pdf") {
                        candidate.photo = await convertPdfToImageFromFileInput([
                            item.getAsFile(),
                        ]);
                    } else {
...
<div class="md:w-4/5 px-2 lg:max-w-screen-lg mx-auto mb-24">
    <section>
        ...
        >
            <p
                class="mb-4 text-2xl font-normal tracking-tight leading-none text-gray-900 md:text-4xl lg:text-5xl dark:text-white"
            >
                Wir bieten:
            </p>
            <h1
            ...
            >
                Den flexibelsten Nebenjob der Stadt
            </h1>
            <p class="font-semibold mb-2 dark:text-white">
                Täglich neue Jobs und Aufgaben als Teil einer familiären Crew
                meistern:
            </p>
            <p class=" dark:text-white">
                Werde ein Teil unserer Crew und bewirb dich noch heute bei uns!
                Wir freuen uns auf deine (Schnell) Bewerbung.
            </p>
        </div>
        <div class="rounded-lg shadow shadow-primary-400 py-3 px-4">
            ...
            >
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            {#if candidate.mobile.length > 3 && !candidate.mobile.match(/\+\d{1,3}\s*\d{3,4}\s*\d{4,8}/)}
                                <Helper class="absolute" color="yellow"
                                    >Formatierung beachten: +[Land] [Vorwahl]
                                    [Nummer] <span class="text-[11px]"
                                        >(z.B. +49 161 123456)</span
                                    ></Helper
                                >
                            {:else if candidate.mobile.length > 0 && !candidate.mobile.match(/\+\d{1,3}/)}
                                <Helper class="absolute" color="red"
                                    >Bitte Ländervorwahl angeben <span
                                        class="text-[11px]">(z.B. +49)</span
                                    ></Helper
                                >
                            {/if}
                        </div>
                    ...
                    <div>
                        <Label for="application_field" class="mb-2"
                            >Einsatzbereich</Label
                        >
                        <Select
                            bind:value={candidate.application_field}
                    ...
                    <div>
                        <Label for="facebook" class="mb-2"
                            >Facebook Profil</Label
                        >
                        <Input
                            bind:value={candidate.facebook}
                            type="text"
                            id="facebook"
                    ...
                    <div>
                        ...
                        <Input
                            bind:value={candidate.instagram}
                            type="text"
                            id="insta"
                    ...
                    <div>
                        <Label for="letter_motivation" class="mb-2"
                            >Du willst ins Team weil...</Label
                        >
                        <Textarea
                            minlength="5"
                            maxlength="700"
                            bind:value={candidate.letter_motivation}
                            rows="9"
                        ...
                        <Helper
                            class="mt-1"
                            color={candidate.letter_motivation.length < 5 ||
                            candidate.letter_motivation.length > 700
                                ? "red"
                                : "green"}
                            >{candidate.letter_motivation.length}/700</Helper
                        >
                    </div>
                    ...
                    <div class="mt-2">
                        ...
                        >
                            {#if !candidate.photo}
                                ...
                                />
                                <Heading class="text-neutral-600" tag="h4"
                                    >Bild hochladen</Heading
                                >
                                <p
                                    class="mb-2 text-sm text-gray-500 dark:text-gray-400"
                                >
                                    <span class="font-semibold">Klicken</span> oder
                                    drag & drop
                                </p>
                                <p
                ...
                <div class="grid gap-3">
                    <Botr bind:valid />
                    <Checkbox required
                        >Ich bin mit der Verarbeitung meiner Daten einverstanden <span
                            class="text-xs pl-2"
                            >(<a
                                class="visited:text-blue-600 hover:text-blue-600 text-blue-500"
